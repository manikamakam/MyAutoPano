...
def main():
	...
	for i in range(5000):
		...
		img  = cv2.imread("../Data/Raw_Train/" + str(i+1)+".jpg")
		...
		A_normal=(patch_A-np.mean(patch_A))/255#(np.std(patch_A) + 10**(-7))
		# print(len(np.where(A_normal > 1)[0]))
		...
		B_normal=(patch_B-np.mean(patch_B))/255#(np.std(patch_B) + 10**(-7))
		...
		savez("../Data/Train/"+str(i+1)+".npz", stacked_img)
