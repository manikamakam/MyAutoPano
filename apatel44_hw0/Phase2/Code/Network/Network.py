...
def CIFAR10Model(Img, ImageSize, MiniBatchSize):
    ...
    net = Img
    net = tf.layers.conv2d(inputs = net, name='layer_conv1', padding='same',filters = 32, kernel_size = 3, activation = tf.nn.relu)
    net  = tf.layers.max_pooling2d(inputs = net, pool_size = 2, strides = 2, name = 'max_pooling1')

    net = tf.layers.conv2d(inputs = net, name = 'layer_conv2', padding= 'same', filters = 64, kernel_size = 3, activation = tf.nn.relu)
    net  = tf.layers.max_pooling2d(inputs = net, pool_size = 2, strides = 2, name = 'max_pooling2')

    net = tf.layers.flatten(net)
    # net = tf.nn.dropout(net, rate = 0.5, name = 'dropout1')
